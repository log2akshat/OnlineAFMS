...
import net.candidate.login.dao.DAORef2Passwd;
...
public class Ref2Passwd extends ActionSupport implements ServletRequestAware
{	
	...
	DAO dao = new DAO();
	DAORef2Passwd daoref2passwd = new DAORef2Passwd();
	...
	User user=(User)session.getAttribute("User");
		...
		if(user!=null)
		{
			   if(user.isCandidate())
			   {
			if(daoref2passwd.send(regid))
			{
				//addActionError(getText("mailsent_referee2"));   
