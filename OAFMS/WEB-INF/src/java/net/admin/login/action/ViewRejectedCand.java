...
import net.admin.login.dao.DAORejectedCand;
...
public class ViewRejectedCand extends ActionSupport implements ServletRequestAware{
	...
	DAORejectedCand dao=new DAORejectedCand();
	...
	public void setRejected(int rejected) {
		//Implementation of the application specific logic to retrieve the data from the database//
			...
			String qry = "Select count(*) from applicant a where Status = 'Rejected'";
			pstmt = connect.prepareStatement(qry);
			result = pstmt.executeQuery();
			{
			        while (result.next())
			        {
			        	RejectedCand = result.getInt(1);
			        	this.rejected = RejectedCand;
			        }
			 }		
			}
